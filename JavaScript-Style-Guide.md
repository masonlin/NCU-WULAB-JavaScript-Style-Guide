...
* 單一空白只出現在以下地方:  
  * 除了 function 及 super 外，用以區隔保留字 (像是 if 、 for 、 或是 catch )，之後於同一行接著開括號 ”(”。
  * 閉大括號 “}” 後用以區隔保留字 (像是 else 或是 catch )。
  * 開大括號 “{” 前，除以下二者例外:  
    * 放在函式的第一個參數或陣例中的第一個元素(例如: foo({a: [{c: d}]}))。  
    * 模板擴展 (例如正規表示式，有效 `ab${1 + 2}cd`，無效 `xy$ {3}z`)。  
  * 任何運算符號二側。
  * 逗號及分號之後。
  * 冒號之後。
  * 陳述式後的註解用的雙斜線 (//) 二側，這裡也允許是多個空隔。
  * 區塊註解的啟始符號 (/**) 之後及其結束符號 (*/) 的二側，例如:
